...
def get_INR(mode,in_features, hidden_features, hidden_layers,
            out_features, outermost_linear=True, high_freq_num=128,low_freq_num=128,phi_num=16,alpha=0.01,first_omega_0=30.0,
            hidden_omega_0=30, pe=False):
    '''
...
# 统一模型接口（适配主程序自动调用）
def get_model(mode='relu+fr', **kwargs):
    return get_INR(mode=mode, **kwargs)
